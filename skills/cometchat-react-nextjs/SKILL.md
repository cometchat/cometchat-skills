...
description: Integrate CometChat React UI Kit v6 into a Next.js project (App Router or Pages Router) using the @cometchat/skills-cli. Replaces the v1 prose skill once the CLI is published to npm.
license: "MIT"
compatibility: "Node.js >=18; React >=18; Next.js 13/14/15/16; @cometchat/chat-uikit-react ^6"
allowed-tools: "executeBash, readFile, fileSearch, listDirectory"
metadata:
  author: "CometChat"
  version: "2.0.0"
  tags: "chat cometchat react nextjs app-router pages-router messaging ui-kit"
---
...
- The project is plain React.js (Vite/CRA) → use `cometchat-react-reactjs`
...
If `framework` is not `"nextjs"`, **stop** and use the correct framework skill.
...
```bash
npx @cometchat/skills-cli@latest view  --experience N --framework nextjs --json
npx @cometchat/skills-cli@latest apply --experience N --framework nextjs --placement dedicated-route
...
- `error` → surface verbatim, do not retry, do not auto-fix

After verify:
- not `pass` → surface failed checks verbatim, stop

### Step 4 — Show install + next steps
...
The CLI has already created `.env.local` with `YOUR_*_HERE` placeholder
values for the three CometChat env vars (APP_ID, REGION, AUTH_KEY) and
added `.env.local` to `.gitignore`. The apply response includes an
`env_file` block confirming this and a `next_steps` array — surface
both verbatim. The user just needs to:

```
Open .env.local and replace the YOUR_*_HERE values with real ones
from https://app.cometchat.com → Your App → API & Auth Keys.
```

Then surface the next_steps array verbatim. The CLI creates the
integration at `/chat`. The user should visit
`http://localhost:3000/chat` after `npm run dev`.
...
> When you open `http://localhost:3000/chat`, here's what to expect
> and how to test:
>
...
- **Always ask the user for choices that are theirs to make** (which experience, which theme preset, which optional feature). Default agent contexts (Claude Code, Cursor, Copilot) are interactive — don't pre-select.
- **Never** ask for choices the CLI already decides deterministically (router type, file paths, package manager). The detect output is the source of truth.
- The user invoking `/cometchat` IS consent to run the deterministic integration steps. Don't ask permission for install/apply/verify.
- Never retry failed commands automatically. Surface the error verbatim.
- Never invent SDK code. Query the docs MCP if you don't know.
