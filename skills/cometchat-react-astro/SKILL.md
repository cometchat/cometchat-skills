...
menu. This reflects the React UI Kit Integration Journey: **init is the
...
>   (Astro caveat: theme overrides must live INSIDE the React island
>   `.tsx` file because `client:only="react"` islands don't pick up
>   global stylesheets. The CLI handles this automatically.)
>
...
>
> - **`d`. Add a floating chat widget** — React island rendered via
>   `<CometChatWidget client:only="react" />` in any `.astro` file.
>   → `npx @cometchat/skills-cli@latest add-widget`
>
> - **`e`. Move from dev auth to production auth** — server-side token
>   endpoint at `src/pages/api/cometchat-token.ts`. Auto-rewrites the
>   client login flow.
>   → `npx @cometchat/skills-cli@latest production-auth`
>
> - **`f`. Server-side user management** — POST/PATCH/DELETE endpoint
>   at `src/pages/api/cometchat-user.ts`.
